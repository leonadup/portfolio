...
const translations = {
    fr: {
        ...
        "proj-serendia-objectifs": "<p>Le projet s'est déroulé en plusieurs phases :</p><ul><li>Phase 1 - Fondations &amp; données : mise en place de l'architecture de l'application et préparation des jeux de données de destinations</li><li>Phase 2 - Moteur de recommandation V1 : conception d'un premier algorithme de recommandation basé sur les préférences utilisateur</li><li>Phase 3 - Moteur amélioré &amp; expérience utilisateur : affinement du moteur de recommandation et amélioration de l'interface et des interactions</li></ul>",
        "proj-serendia-realisation": "<p>Chaque utilisateur est représenté par un <code>UserProfileVector</code>, un vecteur de préférences sur plusieurs axes (Culture, Aventure, Détente, Budget). Les destinations sont comparées à ce profil grâce à une approche de « Soft Filtering » : plutôt que d'exclure strictement les destinations qui ne correspondent pas parfaitement au profil, elles sont simplement pénalisées dans le classement, ce qui permet de garder une diversité de suggestions.</p><p>Un <code>UserInteractionService</code> met à jour ce profil en continu à partir des retours de l'utilisateur, notamment via un mini-jeu de swipe permettant de noter cinq destinations proposées. Les données sont stockées localement dans une base SQLite (via le package <code>sqflite</code>), et des scripts Python (<code>check_db.py</code>, <code>jsonl_to_csv.py</code>) ont été utilisés pour préparer et vérifier les jeux de données de destinations.</p>",
