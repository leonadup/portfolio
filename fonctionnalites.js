...
const translations = {
    fr: {
        ...
        "proj-voronoi-realisation": "<p>En travaillant avec DeepSeek, j'ai obtenu une classe <code>FortuneAlgorithm</code>, héritant d'une classe générique <code>VoronoiGenerator</code>, qui met en œuvre l'algorithme de balayage de Fortune : une ligne de balayage horizontale parcourt le plan et construit progressivement le diagramme à partir des points fournis. Les éléments géométriques (points, arêtes) sont représentés par des classes dédiées <code>Point</code> et <code>Edge</code>, avec des fonctions utilitaires de géométrie regroupées dans <code>geometry_utils</code>.</p><p>Pour visualiser les résultats, des exporteurs dédiés permettent de générer le diagramme final au format <strong>SVG</strong> et <strong>PNG</strong>.</p>",
    ...
    en: {
        ...
        "proj-voronoi-context": "<p>This project is part of a group academic project on Voronoi diagrams, an algorithmic geometry structure used to partition a plane into regions from a set of points. The project was split into three phases, including an individual phase focused on using AI to produce a Voronoi diagram generator: that's the phase detailed below.</p>",
        "proj-voronoi-objectifs": "<p>The goal of this phase was to obtain, by working with the DeepSeek AI, an efficient Voronoi diagram generator, capable of:</p><ul><li>Computing a Voronoi diagram from a set of points using Fortune's algorithm (a sweep line algorithm)</li><li>Representing the cells, vertices and edges of the diagram using dedicated data structures</li><li>Exporting the result as an image (SVG and PNG)</li></ul>",
        "proj-voronoi-realisation": "<p>By working with DeepSeek, I obtained a <code>FortuneAlgorithm</code> class, inheriting from a generic <code>VoronoiGenerator</code> class, which implements Fortune's sweep line algorithm: a horizontal sweep line moves across the plane and progressively builds the diagram from the provided points. Geometric elements (points, edges) are represented by dedicated <code>Point</code> and <code>Edge</code> classes, with utility geometry functions grouped in <code>geometry_utils</code>.</p><p>To visualize the results, dedicated exporters generate the final diagram in <strong>SVG</strong> and <strong>PNG</strong> format.</p>",
        "proj-voronoi-role": "<p>This phase of the project was carried out individually within a group academic project, working with the DeepSeek AI. My role was to guide the AI toward a correct solution: writing the prompts, testing the generated code, spotting cases where the resulting diagram was incorrect, then iterating with the AI until reaching a reliable version of Fortune's algorithm and the associated geometry and export modules, building on the foundations laid during the first phase of the project.</p>",
        "proj-voronoi-resultats": "<p>The result is a generator capable of producing correct Voronoi diagrams from any set of points, exportable directly to SVG or PNG. This project allowed me to deepen my knowledge of algorithmic geometry and complex data structures, while also reflecting on what a generative AI like DeepSeek can (and cannot) produce on the same problem solved \"by hand\" in phase 1.</p>"
    },
    es: {
        // Navigation
        "nav-home": "Inicio",
        ...
        "proj-voronoi-context": "<p>Este proyecto se enmarca en un proyecto académico de grupo sobre los diagramas de Voronoi, una estructura de geometría algorítmica utilizada para dividir un plano en regiones a partir de un conjunto de puntos. El proyecto se desarrolló en tres fases, incluyendo una fase individual dedicada al uso de la IA para producir un generador de diagramas de Voronoi: esa es la fase que detallo a continuación.</p>",
        "proj-voronoi-objectifs": "<p>El objetivo de esta fase era obtener, trabajando con la IA DeepSeek, un generador de diagramas de Voronoi eficiente, capaz de:</p><ul><li>Calcular un diagrama de Voronoi a partir de un conjunto de puntos mediante el algoritmo de Fortune (algoritmo de barrido)</li><li>Representar las celdas, vértices y aristas del diagrama mediante estructuras de datos dedicadas</li><li>Exportar el resultado en forma de imagen (SVG y PNG)</li></ul>",
        "proj-voronoi-realisation": "<p>Trabajando con DeepSeek, obtuve una clase <code>FortuneAlgorithm</code>, que hereda de una clase genérica <code>VoronoiGenerator</code>, que pone en práctica el algoritmo de barrido de Fortune: una línea de barrido horizontal recorre el plano y construye progresivamente el diagrama a partir de los puntos proporcionados. Los elementos geométricos (puntos, aristas) se representan mediante clases dedicadas <code>Point</code> y <code>Edge</code>, con funciones utilitarias de geometría agrupadas en <code>geometry_utils</code>.</p><p>Para visualizar los resultados, exportadores dedicados generan el diagrama final en formato <strong>SVG</strong> y <strong>PNG</strong>.</p>",
        "proj-voronoi-role": "<p>Esta fase del proyecto se realizó de forma individual dentro de un proyecto académico de grupo, trabajando con la IA DeepSeek. Mi papel consistió en guiar a la IA hacia una solución correcta: formular las solicitudes, probar el código generado, detectar los casos en los que el diagrama resultante era incorrecto y luego iterar con la IA hasta obtener una versión fiable del algoritmo de Fortune y de los módulos de geometría y exportación asociados, basándome en lo establecido durante la primera fase del proyecto.</p>",
        "proj-voronoi-resultats": "<p>El resultado es un generador capaz de producir diagramas de Voronoi correctos a partir de cualquier conjunto de puntos, exportables directamente en SVG o PNG. Este proyecto me permitió profundizar mis conocimientos en geometría algorítmica y estructuras de datos complejas, además de reflexionar sobre lo que una IA generativa como DeepSeek puede (y no puede) producir frente al mismo problema resuelto «a mano» en la fase 1.</p>"
    },
    ko: {
        // Navigation
        "nav-home": "홈",
        ...
        "proj-voronoi-context": "<p>이 프로젝트는 보로노이 다이어그램을 주제로 한 그룹 학업 프로젝트의 일부입니다. 보로노이 다이어그램은 점들의 집합을 바탕으로 평면을 영역으로 분할하는 데 사용되는 알고리즘 기하학 구조입니다. 이 프로젝트는 세 단계로 진행되었으며, 그중 한 개인 단계는 AI를 활용해 보로노이 다이어그램 생성기를 만드는 것이었습니다. 아래에서 다루는 내용이 바로 그 단계입니다.</p>",
        "proj-voronoi-objectifs": "<p>이 단계의 목표는 DeepSeek AI와 함께 작업하여 다음을 수행할 수 있는 고성능 보로노이 다이어그램 생성기를 만드는 것이었습니다:</p><ul><li>Fortune 알고리즘(스위프 알고리즘)을 사용하여 점들의 집합으로부터 보로노이 다이어그램 계산하기</li><li>전용 데이터 구조를 사용하여 다이어그램의 셀, 정점, 변을 표현하기</li><li>결과를 이미지(SVG 및 PNG) 형태로 내보내기</li></ul>",
        "proj-voronoi-realisation": "<p>DeepSeek와 함께 작업하여 일반화된 클래스 <code>VoronoiGenerator</code>를 상속하는 <code>FortuneAlgorithm</code> 클래스를 얻었으며, 이는 Fortune의 스위프 알고리즘을 구현합니다: 수평 스위프 라인이 평면을 가로지르며 제공된 점들로부터 점진적으로 다이어그램을 구축합니다. 기하학적 요소(점, 변)는 전용 클래스 <code>Point</code>와 <code>Edge</code>로 표현되며, 기하학 유틸리티 함수들은 <code>geometry_utils</code>에 정리되어 있습니다.</p><p>결과를 시각화하기 위해 전용 내보내기 모듈이 최종 다이어그램을 <strong>SVG</strong> 및 <strong>PNG</strong> 형식으로 생성합니다.</p>",
        "proj-voronoi-role": "<p>프로젝트의 이 단계는 그룹 학업 프로젝트 내에서 DeepSeek AI와 함께 개인적으로 수행되었습니다. 제 역할은 AI를 올바른 해결책으로 이끄는 것이었습니다: 요청 사항을 작성하고, 생성된 코드를 테스트하고, 결과 다이어그램이 잘못된 경우를 찾아낸 다음, 프로젝트 1단계에서 마련된 기반을 바탕으로 Fortune 알고리즘 및 관련 기하학·내보내기 모듈의 신뢰할 수 있는 버전을 얻을 때까지 AI와 함께 반복 작업하는 것이었습니다.</p>",
        "proj-voronoi-resultats": "<p>결과적으로 어떤 점들의 집합으로부터도 올바른 보로노이 다이어그램을 생성할 수 있으며, SVG 또는 PNG로 직접 내보낼 수 있는 생성기를 완성했습니다. 이 프로젝트를 통해 알고리즘 기하학과 복잡한 데이터 구조에 대한 지식을 더욱 심화할 수 있었으며, DeepSeek와 같은 생성형 AI가 1단계에서 '직접 손으로' 해결한 동일한 문제에 대해 무엇을 (그리고 무엇을 할 수 없는지) 만들어낼 수 있는지 돌아볼 수 있었습니다.</p>"
    }
};
